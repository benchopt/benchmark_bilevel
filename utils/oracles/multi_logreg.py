...
class MultiLogRegOracle(BaseOracle):
    ...
    **NOTE:** This class is taylored for the binary logreg.
    ...
    def value(self, theta_flat, lmbda, idx):
        ...
        loss = (-prod[y == 1] + sc.logsumexp(prod, axis=1)).mean(axis=1)
        regul = 0
        if self.reg == 'exp':
            alpha = np.exp(lmbda)
            regul = np.dot(theta_flat, (alpha * theta).ravel())
    ...
    def grad_outer_var(self, theta_flat, lmbda, idx):
        ...
        if self.reg == 'exp':
            alpha = np.exp(lmbda)
            theta = theta_flat.reshape(self.n_features, self.n_classes)
            grad_lmbda = 0.5 * alpha * (theta ** 2).sum(axis=0)
        elif self.reg == 'none':
    ...
    def oracles(self, theta_flat, lmbda, v_flat, idx, inverse="id"):
        ...
        if self.reg == 'exp':
            alpha = np.exp(lmbda)
            loss += 0.5 * np.dot(theta_flat, (alpha * theta).ravel())
            grad_theta += alpha * theta
            hvp += alpha * v
