...
class BaseJaxOracle(ABC):
    ...
    ----------
    cost : callable
        Should take as input (inner_var, outer_var, x, y) and return a scalar.
        Sould be implemented in JAX.
    ...
    def __init__(self, X, y, loss):
        ...
        self.X = X
        self.y = y
        def batched_loss(inner_var, outer_var, idx):
            return jnp.mean(vmap(self.loss, (None, None, 0, 0))(inner_var, outer_var, X, y))
        self.batched_loss = jnp.mean(vmap(self.loss, (None, None, 0, 0)), axis=0)
