...
class Solver(BaseSolver):
    ...
    parameters = {
        ...
        "batch_size, vr": [(1, "saga"), (1, "none"), (32, "none"), (64, "none")],
    }
    ...
    def run(self, callback):
        ...
        if self.step_size == "auto":
            step_size = 1 / self.f_inner.lipschitz_inner(inner_var, outer_var)
        else:
        ...
        lr_scheduler = LearningRateScheduler(
            np.array(step_sizes, dtype=float), np.array(exponents, dtype=float)
        )
        # else:
        ...
        if use_saga:
            memories = init_memory(self.f_inner, self.f_outer, inner_var, outer_var, v)
        else:
...
def init_memory(inner_oracle, outer_oracle, inner_var, outer_var, v):
    memories = _init_memory(inner_oracle, outer_oracle, inner_var, outer_var, v)
    for mem in memories:
...
def saga(
    ...
    for i in range(max_iter):
        ...
        if saga_v:
            ...
            grad_outer = variance_reduction(grad_outer, memory_outer_grad, id_outer)
